...
function drawGraphHierarchichal(g, ver_sep, first_subgraph, second_subgraph) {
    d3.select("#vertices").selectAll("*").remove();
    d3.select("#edges").selectAll("*").remove();
    var n = g.vertices.length;
    var w = 500, h = 500;
...
function updateMatrix(n, arr) {
    ...
    d3.select("#svg_matrix").selectAll(".nnz").style("fill", "none");
    for (var i = 0; i < n; i++) {
        d3.select("#svg_matrix").append("text")
            .style("font", "Arial")
            .style("font-size", "13px")
...
function colorVertices(selected, color) {
    for (var j = 0; j < selected.length; j++) {
        d3.select("#ver" + selected[j]).style("fill", color);
    }
}
